...
@section('main-content')
    ...
    <!-- Page Heading -->
    <h5 class=" mb-4 text-gray-800">{{ __('Staff Dashboard >  Invoices >') }} {{ App\User::find(request()->segment(4))->name}}</h5>
    <hr/>
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"><b>Invoice to:</b> {{App\User::find(request()->segment(4))->name}}<br/>  <b>For:</b> {{$getAssignmentData->getInvoiceData($invoice->assignment_type, $invoice->assignment_hash, 'assignment_title')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                  <form method="post" action="{{route('staffs.show.member.invoices',['client', request()->segment(4),  'assignment' => $invoice->assignment_hash])}}">
                    @csrf

...
@section('title', 'All tax closing assignments')
@section('main-content')

    <!-- Page Heading -->
    <h5 class=" mb-4 text-gray-800">{{ __(ucfirst(request()->segment(1)).' Dashboard >  '.ucfirst(request()->segment(4)).' tax closing assignments') }}</h5>
    <hr/>
              ...
              <tr>
                ...
                <td>
                  ...
                  ?>
                    ...
                    ?>
                <td>
                  <a href="{{route('admins.assignment.tax_closing.view.hash', [$data->hash, strtolower($abc->getStatusByNumber($data->status))])}}" class="btn btn-link">Views</a> | 


                  <a href="#" class="btn btn-link" style="color: {{$btn_color}};" data-toggle="modal" data-target="#invoice_{{$data->hash}}">Invoice</a> | 


                  <a onclick="return confirm('Are you sure you want to delete this item?');" href="{{route('admins.assignment.taxclosing.view.all', request()->segment(4))}}?do=delete&for={{$data->hash}}" style="color: red;">Delete</a>
                </td>
              </tr>
            ...
            <div class="modal fade" id="invoice_{{$data->hash}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <form method="post" action="{{route('admins.assignment.taxclosing.view.all',[request()->segment(4),'assignment' => $data->hash])}}">
                    @csrf
